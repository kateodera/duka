...
class ProductListTestCase(APITestCase):
    def test_list_product(self):
        ...
        self.assertIsNone('count', product_count)
...
class ProductUpdateTestCase(APITestCase):
    def test_update_product(self):
        ...
        self.assertEqual(updated.name, 'New Products')

    def test_upload_product_photo(self):
        product = Product.objects.first()
        original_photo = product.photo
        photo_path = os.path.join(
            settings.MEDIA_ROOT, 'products', 'vitamin-bcomplex.jpg'
        )
        ...
        self.assertNotEquals(response.data['photo'], original_photo)

        try:
            updated = product.pbjects.get(id=product.id)
            expected_photo =os.path.join(
                settings.MEDIA_ROOT,'products', 'vitamin-iron'
            )
            self.assertTrue(
                updated.photo.path.startwith(expected_photo)
            )
        finally:
